...
def resolver(tablero, x, y, paso_actual, soluciones):
    # llega al final
    if (x, y) == (tamano_tablero - 1, tamano_tablero - 1):
        tablero[x][y] = paso_actual
        soluciones.append([fila[:] for fila in tablero])  # guarda copia del tablero
        tablero[x][y] = 0  # borra ultimo paso
        return

    # prueba los movimientos en el orden: arriba, derecha, abajo, izquierda
    for i in range(4):
        nx = x + mov_x[i]
        ny = y + mov_y[i]
        ...
        if es_valido(nx, ny, tablero):
            ...
            resolver(tablero, nx, ny, paso_actual + 1, soluciones)
...
def laberinto():
    ...
    soluciones = []
    resolver(tablero, x_inicial, y_inicial, 1, soluciones)
