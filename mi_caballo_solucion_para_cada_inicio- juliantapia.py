    ...
    for i in range(MAX):
        for j in range(MAX):
            if(tablero[i][j]==contador):
                return i,j
#modulo solucion
def solucion(tablero,fila,columna):
    candidato = 1 ; solucion = False ; x = fila; y = columna; contador = 1
...
for i in range(0,MAX):
    for j in range(0,MAX):
        ...
        if(solucion(tablero,i,j) == True):
            print(f"Hay solucion para la posición ({i},{j})")
            
        else:
            print(f'no hay solucion para la posición ({i},{j})')
        #falta definir 