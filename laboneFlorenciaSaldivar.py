...
def crear_tablero(tamaño):  #función para crear el tablero
...
def buscar_camino(tablero, x, y, paso, soluciones):  #función para buscar un camino en el tablero
    ...
    tablero[x][y]= paso  #marcar la posición con el paso actual
    
    if x == tamano - 1 and y == tamano - 1:  #si se ha llegado al final, guardar la solución
    ...
    for dx, dy in movimientos:  #para cada movimiento posible
        if buscar_camino(tablero, x+dx, y+dy, paso+1, soluciones):  #si se encuentra un camino, devuelve verdadero
            tablero[x][y]=anterior  #restaurar el valor anterior
            return True  #si se encuentra un camino, devuelve verdadero
    tablero[x][y]=anterior  #restaurar el valor anterior
    return False

def resolver_laberinto(tamano):  #función principal para resolver el laberinto
    tablero = crear_tablero(tamano)  #crear el tablero
    agregar_obstaculos(tablero, porcentaje=0.2)  #agregar obstaculos al tablero
    print("Laberinto generado:")  #imprimir el laberinto generado
    ...
    if len(soluciones) == 0:  #si no se encontraron soluciones
        print("No se encontró ningún camino")  #si no se encontraron soluciones
        return
    print("Se encontro:")  
    imprimir_tablero(soluciones[0])  #imprimir la primera solución encontrada

if __name__ == "__main__":  #función principal
    n = int(input("Ingrese el tamaño del laberinto: "))  #pedir al usuario el tamaño del laberinto
    resolver_laberinto(n)  #resolver el laberinto
